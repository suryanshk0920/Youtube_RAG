...
import { getIntro, getYoutubeThumbnail, streamIngest, checkVideoLimit } from "@/lib/api"
...
export function IngestionPanel({ sources, activeKb, onSourcesChange, onIntroReady, onSummarising }: Props) {
  ...
  async function handleIngest(ingestUrl?: string) {
    ...
    try {
      // Check video limit before starting ingestion
      const limitCheck = await checkVideoLimit()
      if (!limitCheck.allowed && limitCheck.upgrade_message) {
        // Reset loading state if limit is reached
        setLoading(false)
        setStep(null)
        setUpgradeModal({
          show: true,
      ...
      // Continue with ingestion if limit check fails
    }
    ...
    try {
      await streamIngest(target, activeKb, {
        ...
        onDone: async (result) => {
          ...
          // Refresh usage after successful ingestion
          refreshUsage()
          try {
